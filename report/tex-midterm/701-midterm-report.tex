...
It is obvious that character plays an important role in affecting the game result. So we may want to

In this character ranking model, we use the game result as the label and the feature vector $\textbf{x}=\{x_1,x_2,\ldots,x_N\}$ to represent the training data. The element $x_i$ indicates the presence of $i$th character in the game, and the detailed value for it is given by
...
\begin{remark}
In the character ranking model, what we want to learn is indeed the ``weight''
\end{remark}

\subsection{Model-II: Group Ranking Model}

To derive the Group Ranking Model, we may start by thinking all the aspects that may have a potential influence on the result of the game. 
